...
public class QuestionsDao {
	...
	public static boolean insertQuestion(Questions q) {
	    ...
	    pst.setString(2, q.getQuestion().trim());
	...
	public static ArrayList<Questions> getAllRecords()
	{
		...
		try
		{
		   ...
		   while(rs.next())
		   {
			  ...
			  s.setQid(rs.getInt(1));
			  s.setQuestion(rs.getString(2));
			  s.setAnswer(rs.getString(7));
			  s.setA(rs.getString(3));
			  s.setB(rs.getString(4));
			  s.setC(rs.getString(5));
			  s.setD(rs.getString(6));
			   samp.add(s);
	...
	public static int deleteRecord(int qid)
	{
		int status = 0;
		try
		{
		...
		Connection con = Provider.getOracleConnection();
		String sql = "delete from questiontable where qid=?";
		PreparedStatement pst = con.prepareStatement(sql);
	    pst.setInt(1, qid);
